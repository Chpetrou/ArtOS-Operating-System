...
int create_default_frame(task_t* task, entry_point_t ep, void* arg)
{
	...
	*stack = (size_t) leave_kernel_task;
	...
	state_size = sizeof(struct registers) - 2*sizeof(size_t);
	stack = (size_t*) ((size_t) stack - state_size);

	stptr = (struct registers *) stack;
...
/** Internally used function to load tasks with a load_args_t structure
...
/** This call is used to adapt create_task calls
...
/** Luxus-edition of create_user_task functions. Just call with an exe name
 *
 * id Pointer to the tid_t structure which shall be filles
 * fname Executable's path and filename
 * argv Arguments list
 * 
 * - 0 on success
